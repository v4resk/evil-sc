...
class hostname(Evasion):
    def __init__(self, platform, args=None):
        super().__init__(platform, args)
        if not args or len(args) == 0:
            raise ValueError("Hostname evasion module requires a hostname argument (-em hostname:computername)")
    ...
    def translate(self):
        ...
        if self.platform == "windows_cs":
            module.components = [
                ...
                EvasionComponent(f"HostCheck{self.uuid}.CheckHostname();")
            ]
